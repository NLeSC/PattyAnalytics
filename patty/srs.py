...
def is_registered(pointcloud):
    ...
    In that case, first transform one pointcloud to the reference system of the
    other, before doing processing on the points:
...
def set_srs(pc, srs=None, offset=np.array([0, 0, 0], dtype=np.float64),
    ...
    Example:
        ...
        # set the SRS to lat/lon, don't use an offset, so it defaults to
        [0,0,0] set_srs( pc, srs="EPSG:4326" )
