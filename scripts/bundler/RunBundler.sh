...
export LD_LIBRARY_PATH=$BASE_PATH/bin:$LD_LIBRARY_PATH
...
then
    ...
    BUNDLETOPMVS=$BASE_PATH/bin/Bundle2PMVS
...
echo "[- Extracting focal lengths -]"
echo $EXTRACT_FOCAL list_tmp.txt
...
echo /bin/sh sift.txt
time /bin/sh sift.txt
...
time $MATCHKEYS list_keys.txt matches.init.txt
...
echo $BUNDLER list.txt --options_file options.txt > bundle/out
time $BUNDLER list.txt --options_file options.txt > bundle/out

echo "[- Generating PMVS files -]"
echo $BUNDLETOPMVS list.txt bundle/bundle.out
time $BUNDLETOPMVS list.txt bundle/bundle.out

echo "[- Preparing PMVS -]"

export BUNDLER_BIN_PATH=$BASE_PATH/bin
echo /bin/sh ./pmvs/prep_pmvs.sh
time /bin/sh ./pmvs/prep_pmvs.sh

echo "[- Running PMVS -]"
echo $PMVS pmvs/ pmvs_options.txt
time $PMVS pmvs/ pmvs_options.txt
